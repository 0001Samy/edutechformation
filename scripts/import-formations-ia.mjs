// Script d'import des formations dans Sanity
// Usage : node --env-file=.env.local scripts/import-formations-ia.mjs
...
if (!token) {
  console.error(
    '❌ SANITY_API_TOKEN manquant.\n' +
      ...
      '   node --env-file=.env.local scripts/import-formations-ia.mjs'
  );
  process.exit(1);
}
...
const formationProductivite = {
  ...
  modules: [
    ...
    module("Utiliser l'IA pour automatiser des tâches simples", [
      ...
      'Ateliers : construire un template IA pour une tâche répétitive — Automatiser un mini-process',
    ]),
...
async function importFormations() {
  const formations = [
    formationIA,
    formationProductivite,
    formationReglementations,
  ];
